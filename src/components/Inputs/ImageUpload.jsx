      ...
      {isDragActive ? (
        ...
        <div>
          ...
          {previewImage && (
            <div className="flex flex-row justify-start items-center gap-4">
              <img
                ...
                className="rounded-xl mt-5 w-16 h-16 object-cover"
              />
          ...
          {previewImageEdit != null && previewImage == null ? (
            <div className="flex flex-row justify-start items-center gap-4">
              <img
                src={CORE_IMAGES_URL + '/uploads/' + previewImageEdit}
                alt="Imagen seleccionada"
                className="rounded-xl mt-5 w-16 h-16 object-cover"
              />
              <p>{previewImageEdit.slice(8)}</p>{' '}
              {/* Slice para quitar el nombre de la carpeta */}
            </div>
          ) : null}
        </div>
      )}
