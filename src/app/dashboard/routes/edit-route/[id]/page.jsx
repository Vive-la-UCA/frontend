...
export default function Page({ params }) {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            const selectedLocationsFetch = await Promise.all(response.locations.map(async location => {
    ...
    const getSelectedLocationHandler = (location) => {
        setFormRoute(prevState => ({ ...prevState, locations: [...prevState.locations, location] }));
    }
    ...
    const removeLocationFromFormRoute = (location) => {
        const newLocations = formRoute.locations.filter(loc => loc.uid !== location.uid);
            ...
            <form onSubmit={submitEditRoute}>
                <div className="flex flex-row justify-between w-full gap-4">
                    <div className="flex flex-col w-1/2">
                        <InputText title={"Nombre"} onChange={getNameHandler} value={formRoute.name} />
                        <Dropdown title={"Localidades"} onClickDropwdown={getSelectedLocationHandler} values={formRoute.locations} onRemoveLocation={removeLocationFromFormRoute} />
                    </div>
                    <ImageUpload onSelectedFile={getImageFileHandler} previewImageEdit={formRoute.image} />
                </div>
